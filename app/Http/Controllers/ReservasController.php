...
class ReservasController extends Controller
{
    public function mostrarR(Request $request) {
        ...
        ->selectRaw('tbl_reservas.*, entrada.nombre_sitio AS entradasitio, salida.nombre_sitio AS salidasitio, DATE_FORMAT(fecha_entrada, "%H:%i") as hora_entrada, DATE_FORMAT(fecha_salida, "%H:%i") as hora_salida')
        ->join('tbl_ubicaciones AS entrada', 'tbl_reservas.ubicacion_entrada', '=', 'entrada.id')
        ->join('tbl_ubicaciones AS salida', 'tbl_reservas.ubicacion_salida', '=', 'salida.id')
        ->where('entrada.empresa', '=', session('empresa'))    
    ...
    public function info(Request $request) {
        ...
        // Realizar un join entre las reservas y las ubicaciones de entrada y salida
        $reserva_cliente = modeloReserva::select('tbl_reservas.*', 'entrada.nombre_sitio as entradasitio', 'salida.nombre_sitio as salidasitio')
            ->join('tbl_ubicaciones as entrada', 'tbl_reservas.ubicacion_entrada', '=', 'entrada.id')
            ->join('tbl_ubicaciones as salida', 'tbl_reservas.ubicacion_salida', '=', 'salida.id')
            ->findOrFail($id_res);
        
        return view('vistas.reserva_cliente', compact('reserva_cliente'));
    }             
