        ...
        <div id="cont-crud" class="collapsed">
            ...
            <div class="menu-content">
                ...
                <button type="button" class="btn btn-outline-light">
                    <a href="{{ route('exportarCSV') }}" style="color: inherit; text-decoration: none;">Exportar CSV</a>
                </button>
                ...
                <div id="lista-parkings">
                    @foreach ($parkings as $parking)
                        <div class="parking-item" id="parking-{{ $parking->id }}">
                            ...
                            <p>Capacidad: {{ $parking->capacidad }}</p>
                            <div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formulario-editar-parking" action="" method="POST">
                        ...
                        <div>
                            <label for="editar-capacidad">Capacidad:</label>
                            <input type="number" name="capacidad" id="editar-capacidad" placeholder="Capacidad..."
                                numeric>
                        </div>
...
@push('scripts')
    ...
    <script>
        ...
        @foreach ($parkings as $parking)
            (function() {
                ...
                obtenerDireccion({{ $parking->latitud }}, {{ $parking->longitud }}, function(direccion) {
                    var popupContent = `<b>{{ $parking->nombre }}</b><br>Dirección: ${direccion}<br><button onclick="verPlazas({{ $parking->id }})">Ver Plazas</button>`;
                    marker.bindPopup(popupContent).openPopup();
                    document.querySelector(`#parking-{{ $parking->id }} p:nth-child(3)`).textContent = `Dirección: ${direccion}`;
                });
        ...
        // Función para abrir la página de plazas del parking
        function verPlazas(parkingId) {
            window.location.href = `/plazasParking?parking=${parkingId}`;
        }
        ...
        function editarParking(id) {
            $.ajax({
                ...
                success: function(parking) {
                    ...
                    $("#editar-capacidad").val(parking.capacidad); // Añadido
        ...
        function verParking(parkingId) {
            if (parkingMarkers[parkingId]) {
                ...
                marker.setIcon(L.divIcon({
                    ...
                    html: '<i class="fas fa-parking" style="font-size: 1.5rem; color: orange; padding: 5px; border: 3px solid black; border-radius: 5px;"></i>',
                    iconSize: [30, 42],
                    iconAnchor: [15, 42],
                }));
